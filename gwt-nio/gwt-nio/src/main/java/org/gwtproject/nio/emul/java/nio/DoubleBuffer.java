 ...
 * A buffer of doubles.
 *
 * <p>A double buffer can be created in either one of the following ways:
 *
 * <ul>
 *   <li>{@link #allocate(int) Allocate} a new double array and create a buffer based on it;
 *   <li>{@link #wrap(double[]) Wrap} an existing double array to create a new buffer;
...
public abstract class DoubleBuffer extends Buffer implements Comparable<DoubleBuffer> {
   ...
   *
   * <p>The new buffer shares its content with this buffer, which means that this buffer's change of
   ...
   *
   * <p>The remaining doubles will be moved to the head of the buffer, staring from position zero.
   ...
   *
   * <p>If {@code other} is not a double buffer then {@code false} is returned. Two double buffers
   * are equal if and only if their remaining doubles are exactly the same. Position, limit,
   ...
   *     dest.length}.
   * @param len the number of doubles to read, must be no less than zero and not greater than {@code
   *     dest.length - off}.
   * @return this buffer.
   ...
   *
   * @return {@code true} if this buffer is based on a double array and provides read/write access,
  ...
  /**
   * Writes doubles from the given double array to the current position and increases the position
   ...
   * position and increases the position by the number of doubles written.
   *
   * @param src the source double array.
