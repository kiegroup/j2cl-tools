 ...
 * A buffer of floats.
 *
 * <p>A float buffer can be created in either of the following ways:
 *
 * <ul>
 *   <li>{@link #allocate(int) Allocate} a new float array and create a buffer based on it;
 *   <li>{@link #wrap(float[]) Wrap} an existing float array to create a new buffer;
 *   <li>Use {@link java.nio.ByteBuffer#asFloatBuffer() ByteBuffer.asFloatBuffer} to create a float
...
public abstract class FloatBuffer extends Buffer implements Comparable<FloatBuffer> {
   ...
   *
   * <p>The remaining floats will be moved to the head of the buffer, starting from position zero.
   ...
   * duplicated buffer's read-only property and byte order are same as this buffer too.
   *
   ...
   *
   * <p>If {@code other} is not a float buffer then {@code false} is returned. Two float buffers are
   * equal if and only if their remaining floats are exactly the same. Position, limit, capacity and
   ...
   *     dest.length}.
   * @param len the number of floats to read, must be no less than zero and no greater than {@code
   *     dest.length - off}.
   * @return this buffer.
   ...
   *
   * @return {@code true} if this buffer is based on a float array and provides read/write access,
  ...
  /**
   * Writes floats from the given float array to the current position and increases the position by
  ...
  /**
   * Writes floats from the given float array, starting from the specified offset, to the current
   * position and increases the position by the number of floats written.
   *
   * @param src the source float array.
   * @param off the offset of float array, must not be negative and not greater than {@code
   *     src.length}.
   * @param len the number of floats to write, must be no less than zero and no greater than {@code
   *     src.length - off}.
   * @return this buffer.
