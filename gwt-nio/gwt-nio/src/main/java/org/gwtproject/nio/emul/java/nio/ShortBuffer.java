 ...
 * A buffer of shorts.
 *
 * <p>A short buffer can be created in either of the following ways:
 *
 * <ul>
 *   <li>{@link #allocate(int) Allocate} a new short array and create a buffer based on it;
 *   <li>{@link #wrap(short[]) Wrap} an existing short array to create a new buffer;
...
public abstract class ShortBuffer extends Buffer implements Comparable<ShortBuffer> {
   ...
   *
   * <p>The offset is the index of the array corresponding to the zero position of the buffer.
   *
   ...
   *
   * <p>The remaining shorts will be moved to the head of the buffer, starting from position zero.
   ...
   *
   * <p>If {@code other} is not a short buffer then {@code false} is returned. Two short buffers are
   * equal if and only if their remaining shorts are exactly the same. Position, limit, capacity and
   ...
   *     dest.length}.
   * @param len the number of shorts to read, must be no less than zero and not greater than {@code
   *     dest.length - off}.
   * @return this buffer.
   ...
   *
   * @return {@code true} if this buffer is based on a short array and provides read/write access,
  ...
  /**
   * Writes shorts from the given short array to the current position and increases the position by
  ...
  /**
   * Writes shorts from the given short array, starting from the specified offset, to the current
   * position and increases the position by the number of shorts written.
   *
   * @param src the source short array.
   * @param off the offset of short array, must not be negative and not greater than {@code
   *     src.length}.
   * @param len the number of shorts to write, must be no less than zero and not greater than {@code
   *     src.length - off}.
   * @return this buffer.
