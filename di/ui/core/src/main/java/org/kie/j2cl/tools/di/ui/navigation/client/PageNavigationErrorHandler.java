...
public interface PageNavigationErrorHandler {
   ...
   */
  void handleError(Exception exception, Class<? extends UniquePageRole> pageRole);

  /**
   *
   * @param exception The exception that occurs, triggering the error handler code.
   * @param urlPath The URL path which we tried to navigate to.
   */
  void handleInvalidURLError(Exception exception, String urlPath);
}
